...
def train_net(net,
    ...
    train = BasicDataset(train_img, train_mask, img_scale) # input, label, scale
    n_train = len(train)
    val = BasicDataset(val_img, val_mask, img_scale) # input, label, scale
    n_val = len(val)
...
def get_args():
    ...
    parser.add_argument('-f', '--load', dest='load', type=str, default=False,   #dir 파일 경로 위쪽에 선언해둠
...
if __name__ == '__main__':
    ...
    net = UNet(n_channels=3, n_classes=1, bilinear=True) #RGb 학습
    # net = UNet(n_channels=1, n_classes=1, bilinear=True) #gray 학습
